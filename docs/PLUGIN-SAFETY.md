...
自动回复规则:
  ...
  回复内容: {prefix}pt {1}
  冷却: 30 秒
模块 commands: 本账号执行 pt_promote 置顶
```
...
冷却中不再静默跳过：自动回复会提示该用户今日已使用次数、每日上限和剩余 CD，例如“今日已置顶 1/2 次，本次是第 1 次，距离下次可用 CD 还剩 6小时”。当本次已达到每日上限时，会提示“本次是第 2 次，当日无法再次使用置顶功能，如需使用请联系管理员或明日再用”。管理员可回复某个群友的消息发送 `{prefix}arcd`，也可发送 `{prefix}arcd 123456789` 按 Telegram 用户 ID 重置当前会话相关的会话/用户冷却和该用户今日次数；如果只想重置某条规则，追加规则 ID。
