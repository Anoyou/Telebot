...
远程插件、官方可选插件和核心平台能力共用同一套 `Plugin` / `Manifest` / `PluginContext` 规范。远程插件额外需要 `plugin.json` 作为安装阶段静态元数据，并由 Git 仓库安装到 `plugins/installed/{name}/`；官方可选插件同样通过 Web 安装到 `plugins/installed/{name}/` 后运行。
