...
远程插件、官方可选插件和核心平台能力共用同一套 Plugin API、`Manifest`、`PluginContext`、能力 facade、配置 schema、插件分类和交互 Bot 声明。区别在安装来源：远程插件从 Git 仓库安装到 `plugins/installed/{name}/`，官方可选插件从随包 `backend/app/worker/plugins/official/{name}/` 复制到 `plugins/installed/{name}/`，安装阶段先静态读取 `plugin.json`，不会执行远程 Python；运行阶段再由 worker loader 加载 `manifest.py` / `plugin.py`。
...
- 安装型插件使用受控客户端 facade，按 `permissions` 决定可见能力；这不是公共市场式强沙箱，插件业务风险仍由管理员自行承担。
...
| `message_channels` | 推荐 | 不同调度方式的默认消息通道，例如管理员命令走 `userbot_reply`，群内玩法走 `interaction_bot` |
| `money_channel` | 推荐 | 钱相关动作通道，当前固定写 `userbot_reply` |
...
1. 进入插件中心的远程插件页面。
2. 输入 Git 仓库地址；如果要从临时分支安装，可以直接粘贴 GitHub 分支页链接，例如 `https://github.com/user/repo/tree/feature-demo`。
3. 点击安装。
...
插件仓库列表支持 GitHub 私有仓库。推荐做法是把仓库 URL 和凭证分开保存：

- 仓库 URL 仍填写干净的 `https://github.com/user/repo.git`，不要把 token 拼进 URL。
- 临时分支也可以直接填写 GitHub 分支页 `https://github.com/user/repo/tree/<branch>`；后端会保留原始 URL 用于展示和凭证匹配，拉取时转换成 `https://github.com/user/repo.git` 并 checkout 指定分支。
- GitHub Token 使用 fine-grained token，至少授予目标仓库 `Contents: Read` 权限。
- Token 会用 TelePilot `MASTER_KEY` 加密保存，API 和前端只返回 `has_credentials=true/false`，不会回显明文。
- 拉取仓库时，后端通过临时 git extraheader 注入 token，不会写入 git remote、缓存目录名或日志。
- 如果 `MASTER_KEY` 变更导致凭证无法解密，需要恢复原 `MASTER_KEY` 或在插件仓库页重新保存 token。

SSH 私有仓库仍可以用 `git@github.com:user/repo.git` 或 `git+ssh://...`，但需要你在服务器/容器环境里自行配置可用 SSH key；Web 表单里的 GitHub Token 只用于 `https://github.com/...` 仓库。
...
| `POST` | `/api/remote-plugins/install` | 从 Git URL 安装；支持 GitHub `tree/<branch>` 分支页 |
...
| `GET` | `/api/plugin-repos` | 列出已保存插件仓库；只返回是否已配置凭证 |
| `POST` | `/api/plugin-repos` | 保存插件仓库；支持 GitHub `tree/<branch>` 分支页，可带 `credential.token` 添加私有 GitHub 仓库 |
| `PUT` | `/api/plugin-repos/{id}/credential` | 更新或清除插件仓库凭证 |
| `GET` | `/api/plugin-repos/{id}/plugins` | 浏览仓库内插件 |
| `POST` | `/api/plugin-repos/{id}/refresh` | 强制刷新仓库缓存并返回插件列表 |
| `POST` | `/api/plugin-repos/{id}/plugins/{name}/install` | 从仓库安装指定插件 |

### 权限、审计与代发

TelePilot 标准模式按个人可信插件处理：管理员安装并启用远程插件后，插件业务风险由管理员承担。平台仍会通过 facade 代发、记录日志、执行频控和保留急停能力；插件不直接拿 Bot Token 或 userbot session。
...
- [ ] 返回动作的 `send_via` 使用平台受控通道：`interaction_bot`、`userbot_reply`、`bbot_notice`；需要主动收窄时再写 `result_contract.send_via`。
