...
TelePilot 标准模式是个人可信插件模式：管理员安装并启用插件后，视为信任插件业务逻辑；平台保留频控、审计、急停和 token/session 隔离。交互 Bot 用来承接群内高频互动，UserBot 负责全量监听、管理员命令、收款确认和发奖。后续插件要支持“转账命中后启动”或“群友关键词开局”，应通过 Manifest 声明一个或多个交互入口，并实现 `on_interaction` 返回平台标准动作。
...
这里的三角联动，指的是通知 Bot 负责群内公告和规则触发，交互 Bot 负责承接高频互动，`UserBot` 负责最后一段账号动作。`UserBot` 不是拿来跑高频游戏的，也不是拿来直接接收所有群消息的，它只需要稳定监听通知 Bot 的公告和命中结果，在需要发奖或补发时按公告去回复即可。
...
原则上，普通 Bot 不碰钱，只负责公告、命中、对账提示和规则事件；真正的奖金发放仍由 `UserBot` 根据结构化公告去回复完成。插件如果要做发奖、结算或红包类动作，也应把钱相关动作留在 `UserBot` 侧，不要把转账、发奖、催付这些动作混进交互 Bot 的高频入口里。
...
交互 Bot 运行时采用事件路由模型：普通 Bot 负责接收群消息、按钮回调和规则指令；UserBot/通知 Bot 链路负责补充付款与公告证据；平台只把命中规则且存在活跃会话的事件投递给对应插件，不会把所有群消息广播给所有插件。插件应在同一个 `on_interaction` 中按 `payload["event"]["type"]` 区分事件。
...
| `send_message` | `reply_markup` | 可选，Bot API inline keyboard；只会透传给 `interaction_bot` / `bbot_notice`，`userbot_reply` 不承接按钮 |
...
通道原则是：**插件拥有通道选择权，框架拥有通道执行权**。插件可以选择单一通道，也可以声明候选通道；平台负责检查入口白名单、能力边界、按钮限制、频控、审计和实际发送。推荐只使用这些通道值：
...
| `bbot_notice` | 由通知 Bot 发公告、命中、对账提示 | 不处理钱相关执行动作；别名 `notice` |
| `auto` | 按平台默认候选顺序尝试 | 当前等价于 `interaction_bot -> userbot_reply -> bbot_notice`，仍受 `result_contract.send_via` 限制 |

入口未声明 `result_contract.send_via` 时，平台按可信插件标准允许 `interaction_bot`、`userbot_reply`、`bbot_notice` 三个受控通道。入口声明了 `result_contract.actions` 时，运行时会丢弃未声明动作；显式收窄 `send_via` 后，不在白名单内的候选通道会被过滤，全部不命中时动作会被丢弃并写入运行时日志。`reply_markup` 只会透传给 `interaction_bot` / `bbot_notice`，若候选通道包含 `userbot_reply`，平台会自动收窄到可承接按钮的 Bot 通道；只有 userbot 候选时会移除按钮。涉及奖金、补发、转账、催付的插件必须在 `settlement` 中写清职责：普通 Bot 只能公告和给出可对账结果，真正收款确认和发奖仍由账号 worker 的 userbot 代发或由平台受控结算流程处理。
...
`payload_contract` 描述输入，`result_contract` 描述输出。它们是文档化契约，也可以作为插件主动收窄动作范围的运行时守卫依据。`result_contract.actions` 只能列标准动作；`result_contract.send_via` 是可选发送者白名单；`settlement` 只说明结算/公告语义，不能让普通 Bot 直接拥有发奖权限。
...
3. 插件不处理 Bot Token、通知 Bot 原文格式、转账过滤、发奖账号；这些都属于平台层职责，钱相关动作也不该放进交互 Bot 的高频入口。
