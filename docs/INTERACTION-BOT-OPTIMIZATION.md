...
5. 新插件作者只要遵守规范，就能低成本接入 TelePilot 的双通道调度：入口描述管理员命令或群内玩法的触发来源和默认偏好，插件实际动作可以选择 userbot / 交互 Bot / 通知 Bot 的单通道或候选顺序，资金动作仍走 userbot / 平台受控结算。
...
动作可带 `send_via`，也可以通过 `channel`、`channel_selector` 或 `send_via_options` 声明候选顺序，但最终必须命中入口声明的白名单。当前推荐白名单只有：
...
运行时会按 `result_contract` 执行守卫：未声明 `send_via` 时按个人可信插件标准允许 `interaction_bot`、`userbot_reply`、`bbot_notice` 三个受控通道；声明了 `actions` 时，未声明动作会被丢弃；显式收窄 `send_via` 后，不在白名单内的候选通道会被过滤，全部不命中时写入 runtime log；`reply_markup` 只透传给 `interaction_bot` / `bbot_notice`，候选中包含 `userbot_reply` 时会自动收窄到可承接按钮的 Bot 通道。
...
2. **插件声明允许的接收形态与发送通道偏好/候选顺序**，平台负责执行、过滤、审计和回退。
3. 如有必要，可在入口声明层新增类似：
   - `accepted_sources`
...
8. `send_via` / `channel` / `channel_selector` 只能命中声明白名单；候选通道由平台按顺序尝试和回退。
...
- 已落地统一 payload 信封。
- 已落地统一动作结果。
- 已落地 `result_contract.actions` 与 `result_contract.send_via` 运行时守卫；未主动收窄时按可信插件标准允许三通道代发，并支持插件声明候选通道与失败回退。
